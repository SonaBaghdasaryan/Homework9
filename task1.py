...
def decor_csv(function: Callable):
    coefficients_csvfile()
...
def json_result(func: Callable):
    result = {}
    if os.path.exists('equation_file.json'):
        with open('equation_file.json', 'r', encoding='UTF-8') as file:
            result = json.load(file)
    else:
        with open('equation_file.json', 'w', encoding='UTF-8') as file:
    ...
    print("Решение в файле:  equation_file.json")
    return wrapper


def coefficients_csvfile():
